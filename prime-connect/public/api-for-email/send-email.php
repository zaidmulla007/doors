 ...
 * 1. Upload this file to public_html/api-for-email/
 * 2. Upload config.php to public_html/api-for-email/ (created from config.example.php)
 * 3. Install PHPMailer:
 *    Option A (Easy): Download PHPMailer ZIP from https://github.com/PHPMailer/PHPMailer/archive/refs/heads/master.zip
 *                     Unzip and rename folder to 'PHPMailer' inside 'api-for-email' folder.
 *                     Structure should be: public_html/api-for-email/PHPMailer/src/...
 *    Option B (Composer): Run 'composer require phpmailer/phpmailer' in api-for-email directory
 */
...
$subject = htmlspecialchars(strip_tags($input['subject'] ?? ''), ENT_QUOTES, 'UTF-8');
$message = htmlspecialchars(strip_tags($input['message'] ?? ''), ENT_QUOTES, 'UTF-8');

// Optional fields for product inquiry
$productName = htmlspecialchars(strip_tags($input['productName'] ?? ''), ENT_QUOTES, 'UTF-8');
$formType = htmlspecialchars(strip_tags($input['formType'] ?? 'contact'), ENT_QUOTES, 'UTF-8'); // contact, consultation, product-inquiry
...
try {
    ...
    $mail->addAddress(SMTP_TO_EMAIL);      // Primary recipient
    $mail->addAddress(SMTP_TO_EMAIL_2);    // Secondary recipient
    ...
    if ($formType === 'consultation') {
        $emailSubject = "New Consultation Request: " . $name;
    } elseif ($formType === 'product-inquiry' && $productName) {
        $emailSubject = "Product Inquiry: " . $productName;
    }
    ...
    if ($formType === 'consultation') {
        $emailBody = "<h2>New Consultation Request</h2>";
    } elseif ($formType === 'product-inquiry') {
        $emailBody = "<h2>New Product Inquiry</h2>";
        if ($productName) {
            $emailBody .= "<p><strong>Product:</strong> {$productName}</p>";
        }
    }
    ...
    // Alt body for plain text
    $altBody = "Name: $name\nEmail: $email\nPhone: $phone\nSubject: $subject\nMessage: $message";
    if ($productName) {
        $altBody = "Product: $productName\n" . $altBody;
    }
